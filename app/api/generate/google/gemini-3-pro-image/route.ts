...
export async function POST(request: NextRequest) {
  try {
    ...
    let referenceImage = formData.get('referenceImage') as File | null;
    ...
    if (templateSlug) {
      ...
      if (template.useImage) {
        ...
        // Set reference options to copy everything from the template image
    ...
    if (templateImageUrl) {
      ...
      try {
        const base64Data = await fetchImageAsBase64(templateImageUrl);
        contents.push({
          inlineData: {
            mimeType: 'image/jpeg',
            data: base64Data,
        ...
        console.log('Added template image to contents');
      } catch (err) {
    ...
    for (let i = 0; i < imageCount; i++) {
      try {
        console.log(
          'Generating image with Gemini 3 Pro Image, prompt:',
          enhancedPrompt
        );
